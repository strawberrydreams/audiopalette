...
BUNDLE_PATH = BASE_DIR / "model" / "attention_rnn.mag"

# 번들 및 제너레이터 전역 초기화
try:
    bundle = sequence_generator_bundle.read_bundle_file(str(BUNDLE_PATH))
    generator = melody_rnn_sequence_generator.get_generator_map()['attention_rnn'](
        checkpoint=None, bundle=bundle
    )
    generator.initialize()
    logger.info("Magenta ATTENTION RNN 모델이 성공적으로 초기화되었습니다.")
...
def generate_magenta_attention_midi(
    prompt: str,
    qpm: float = 120.0,
    duration_secs: float = 4.0
) -> str:
    """
    Magenta ATTENTION RNN 모델을 사용해 시퀀스를 생성하고 MIDI 파일로만 저장합니다.
    ...
    filename_base = f"{timestamp}_{safe_prompt}_magenta_attention"
    output_dir = BASE_DIR / "magenta-outputs" / "attention_rnn"
