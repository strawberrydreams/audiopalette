# MusicGen Small 모델의 Handler 파일 (클라우드)
...
# 파일 실행 시 한 번만 호출되어 MusicGen Small 모델을 device에 올려 둠
# Args: device: 명시적 torch.device 지정. None이면 _DEVICE_DEFAULT 사용.
# Returns: 캐싱된 MusicGen 인스턴스를 반환함
def load_musicgen_small(device: Optional[torch.device] = None) -> MusicGen:
    global _MODEL_CACHE
...
# 입력받은 프롬프트를 기반으로 MusicGen Small 모델을 구동함
...
def generate_musicgen_small_wav(
    prompt: str, 
    *,
    ...
    model = load_musicgen_small(device)
...
if __name__ == "__main__":
    ...
    path = generate_musicgen_small_wav(TEST_PROMPT, gen_params={"duration": 8})
